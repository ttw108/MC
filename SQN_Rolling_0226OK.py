...
def pca_f(df_, color_list, label_dict):
    ...
    for i, strategy in enumerate(strategies):
        ...
        if strategy in label_dict:
            label = strategy
            plt.annotate(label, (pca.components_[0][i], pca.components_[1][i]), color='red', fontsize=24)
        else:
            label = strategy
        plt.annotate(label, (pca.components_[0][i], pca.components_[1][i]), color='green', fontsize=14)
...
def tsne_f(df_, target_col, labels,sqnlis):
    ...
    for i, label in enumerate(labels):
        ...
        fs=24 if label in sqnlis else 14
        plt.text(df_tsne.iloc[i]['X'], df_tsne.iloc[i]['Y'], label, fontsize=fs, color=color)
    plt.title('t-SNE plot')
    plt.show()
labels=list(dfa.columns)
tsne_f(dfaa,','.join(dfa.columns),labels,sqn_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
pic=sht_pic.pictures.add(pic_path, name='dfaa_t', left=sht_pic.range('A19').left, top=sht_pic.range('A19').top)
pic.height *= 0.77
pic.width *= 0.77
################################################################
tsne_f(dfab,','.join(dfa.columns),labels,sqn_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
pic=sht_pic.pictures.add(pic_path, name='dfab_t', left=sht_pic.range('I19').left, top=sht_pic.range('I19').top)
pic.height *= 0.77
pic.width *= 0.77

################################################################
tsne_f(dfac,','.join(dfa.columns),labels,sqn_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
pic=sht_pic.pictures.add(pic_path, name='dfac_t', left=sht_pic.range('Q19').left, top=sht_pic.range('Q19').top)
pic.height *= 0.77
pic.width *= 0.77
################################################################
################################################################
...
################################################################
################################################################
umap_f(dfab, ','.join(dfa.columns), labels,sqn_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
...
################################################################
################################################################
umap_f(dfac, ','.join(dfa.columns), labels,sqn_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
